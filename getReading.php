...
if ($db->connect_errno > 0) {
    ...
    if ($type == "lastHour") {
        ...
        $sql = "select date_created as time_created,SUM(elster)/4 as average_elster from (select distinct * from elster_readings where `date_created` >= DATE_SUB( NOW(), INTERVAL 30 DAY ) AND `date_created` <= NOW()) as foo group by DATE(date_created), 4*floor(date_format(date_created,'%H')/4)";
